...
def main():
    ...
    ensemble = utils.get_models(args, train=False, as_ensemble=True, model_file=args.model_file, leaky_relu=leaky_relu,
                                is_paral='trs' in args.model_file)
    ...
    if args.subset_num:
        ...
        ########batch_sizeÔ­Îª1000
        testloader = utils.get_testloader(args, batch_size=args.test_size, shuffle=False, subset_idx=subset_idx)
    else:
        testloader = utils.get_testloader(args, batch_size=args.test_size, shuffle=False)
    ...
    if args.convergence_check:
        ...
        # _, label, pred, advpred = attack_whole_dataset(adversary, test_iter, args.isSM,device="cuda")
        ...
        for steps in tqdm(steps_list, desc='PGD steps', leave=False, position=0):
            ...
            for i in tqdm(range(random_start), desc='Random Start', leave=False, position=1):
                ...
                adversary = MomentumIterativeAttack(predict=ensemble, loss_fn=loss_fn,
                                                    eps=eps, nb_iter=steps, eps_iter=eps / 5,
                                                    decay_factor=1,
        ...
        # save to file
        if args.save_to_csv:
            output_root = os.path.join('results', 'mnist', 'wbox', train_alg, train_seed, 'convergence_check')
            if args.dataset_name == "FashionMNIST":
                output_root = os.path.join('results', 'fashioMNIST', 'wbox', train_alg, train_seed, 'convergence_check')
        ...
        for eps in tqdm(eps_list, desc='PGD eps', leave=True, position=0):
            ...
            for i in tqdm(range(args.random_start), desc='Random Start', leave=False, position=1):
                ...
                test_iter = tqdm(testloader, desc='Batch', leave=False, position=2)
                adversary = MomentumIterativeAttack(predict=ensemble, loss_fn=loss_fn,
                                                    eps=eps, nb_iter=args.steps, eps_iter=eps / 5,
                                                    decay_factor=1,
                                                    clip_min=0.0, clip_max=1.0, targeted=False)
            ...
            # save to file
            if args.save_to_csv:
                output_root = os.path.join('results', 'mim', train_alg, train_seed)
                if not os.path.exists(output_root):
                    os.makedirs(output_root)
                output_filename = args.model_file.split('/')[-2]
                output = os.path.join(output_root, '.'.join((output_filename, 'csv')))
