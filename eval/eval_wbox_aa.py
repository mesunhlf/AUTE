    ...
    if args.convergence_check:
        ...
        for eps in tqdm(eps_list, desc='PGD eps', leave=True, position=0):
            ...
            for i in tqdm(range(args.random_start), desc='Random Start', leave=False, position=1):
                ...
                adversary = AutoAttack(ensemble, norm='Linf', eps=eps, version='standard', verbose=False)
                adversary.attacks_to_run = ['apgd-ce']
                for batch_idx, (images, label) in enumerate(test_iter):
                    time_s = time.time()

                    images, label = images.cuda(), label.cuda()

                    x_adv = adversary.run_standard_evaluation(images, label, bs=2000)
                    outputs = ensemble(x_adv)
                    _, pred = outputs.max(1)
                    total_time += (time.time() - time_s)
                    correct_or_not.append(label == pred)

                    c_count += np.sum(np.array((label == pred).tolist()))

            correct_or_not = torch.stack(correct_or_not, dim=-1).all(dim=-1)

            tqdm.write("eps: {:.2f}, Adv Accuracy: {:.2f}%".format(
                eps,
                100. * c_count / 10000))
        ...
        # save to file
        if args.save_to_csv:
            output_root = os.path.join('results', 'wbox_aa', train_alg, train_seed)
            if not os.path.exists(output_root):
                os.makedirs(output_root)
            output_filename = args.model_file.split('/')[-2]
            output = os.path.join(output_root, '.'.join((output_filename, 'csv')))
