...
def gradient_wrt_input2(model, others, inputs, targets, criterion=nn.CrossEntropyLoss()):
    inputs.requires_grad = True
...
def Linf_distillation(model, dat, target, eps, alpha, steps, layer, before_relu=True, mu=1, momentum=True,
                      rand_start=False):
    x_nat = dat.clone().detach()
    x_adv = None
    # print(rand_start)
